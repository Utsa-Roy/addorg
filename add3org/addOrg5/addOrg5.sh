...
function Org5Up () {
  ...
  if [ "${DATABASE}" == "couchdb" ]; then
    DOCKER_SOCK=${DOCKER_SOCK} docker-compose -f $COMPOSE_FILE_ORG5 -f $COMPOSE_FILE_COUCH_ORG5 up -d 2>&1
  else
    DOCKER_SOCK=${DOCKER_SOCK} docker-compose -f $COMPOSE_FILE_ORG5 up -d 2>&1
  fi
  if [ $? -ne 0 ]; then
